...
function inferTagDescription(tagName) {
  ...
  const descriptions = {
    ...
    courses: 'Courses (الدورات): catalog, details, and course-level enrollment endpoints.',
    enrollments: 'Course subscriptions (اشتراكات الدورات) and lesson progress tracking endpoints.',
    ...
    'student-subscriptions': 'Student Quran live-session subscriptions, package selection, and renewal endpoints.',
    ...
    'quran-sheikhs': 'Quran live-session sheikhs (FULL_TEACHER): listing, availability, schedules, and reviews. Course-only sheikhs (COURSE_SHEIKH) are excluded.',
    student: 'Student flow: sessions, reports, my courses, and search for sheikhs and courses with pagination.',
...
function inferRequestExample(routePath, method) {
  ...
  if (p.includes('/v1/enrollments/') && p.includes('/fawry/reference-number')) {
    return { expiryHours: 24, language: 'ar-eg' };
  }
  if (p.includes('/fawry/reference-number')) {
    return { bookingId: 'clx123exampleid', expiryHours: 24, language: 'en-gb' };
  }
  if (p.includes('/v1/enrollments/lessons/') && p.includes('/start') && m === 'post') {
    return { videoId: 'clxvideo123' };
  }
  if (p.includes('/v1/enrollments/lessons/') && p.includes('/complete') && m === 'post') {
    return { videoId: 'clxvideo123', watchDurationSeconds: 1800 };
  }
  if (p.includes('/v1/enrollments/') && p.endsWith('/enroll') && m === 'post') {
    return { sheikId: 'clxsheikh123' };
  }
  if (p.includes('/student-subscriptions/subscribe') && m === 'post') {
    return {
      packageId: 'sub_pkg_monthly_quran_8',
      teacherId: 'teacher_quran_ahmed_hassan',
      selectedSlots: [
        { scheduleId: 'schedule_sat_0330_0530' },
  ...
  if (p.includes('/quran-sheikhs/me/schedules') && m === 'post') {
    return {
      slots: [
        { dayOfWeek: 6, startTime: '03:30', endTime: '05:30' },
  ...
  if (p.includes('/student-subscriptions/packages') && (m === 'post' || m === 'put')) {
    return {
      name: 'Quran Live 8 Sessions',
      nameAr: 'باقة القرآن المباشرة - 8 حصص',
      description: '8 live Quran sessions per month with a dedicated sheikh.',
      descriptionAr: 'ثماني حصص قرآن مباشرة شهريا مع شيخ مخصص.',
      price: 1600,
      durationMonths: 1,
      sessionsPerMonth: 8,
      maxBookings: 8,
      isActive: true,
      isPopular: true,
      features: ['8 live sessions', 'Flexible rescheduling'],
      featuresAr: ['8 حصص مباشرة', 'مرونة في إعادة الجدولة'],
    };
  }
...
function inferResponseDataExample(routePath, method) {
  ...
  if (p.includes('/student-subscriptions/subscribe') && m === 'post') {
    return {
      ...
      reservation: {
        slotsPerWeek: 2,
        totalReservedSessions: 8,
        bookingStatus: 'PENDING',
      },
      paymentId: 'pay_fawry_sub_001',
      merchantRefNum: 'pay_fawry_sub_001',
      referenceNumber: '987654321012',
      paymentUrl: 'https://atfawry.fawrystaging.com/ECommerceWeb/Fawry/payments/charge?ref=pay_fawry_sub_001',
      statusCode: 200,
      statusDescription: 'Operation done successfully',
    };
  }
  if (m === 'get' && (p.includes('/v1/quran-sheikhs/me/schedules') || p.includes('/mobile/quran-sheikhs/me/schedules'))) {
    return {
      teacherId: 'teacher_quran_ahmed_hassan',
      total: 2,
      schedules: [
  ...
  if (m === 'get' && p.includes('/student-subscriptions/packages')) {
    return [
      {
        id: 'sub_pkg_monthly_quran_8',
        name: 'Quran Live 8 Sessions',
        price: 1600,
        durationMonths: 1,
        sessionsPerMonth: 8,
  ...
  if (m === 'get' && (p.includes('/v1/quran-sheikhs/') || p.includes('/mobile/quran-sheikhs/')) && p.includes('/availability')) {
    return {
      teacherId: 'teacher_quran_ahmed_hassan',
      range: {
      ...
      days: [
        ...
        },
        {
          ...
          availableWindows: [
            {
              scheduleId: 'schedule_sat_0330_0530',
              slotId: 'schedule_sat_0330_0530:2026-02-28:03:30',
              startTime: '03:30',
              endTime: '05:30',
            },
          ],
  ...
  if (m === 'get' && (p.includes('/v1/quran-sheikhs/') || p.includes('/mobile/quran-sheikhs/'))) {
    if (p.endsWith('/bookable') || p === '/v1/quran-sheikhs/' || p === '/mobile/quran-sheikhs/' || p === '/v1/quran-sheikhs' || p === '/mobile/quran-sheikhs') {
      return {
        sheikhs: [
          {
            id: 'teacher_quran_ahmed_hassan',
            name: 'Ahmed Hassan',
            title: 'Quran reciter and memorization',
            specialization: 'Tajweed',
            image: 'https://shike.developteam.site/uploads/sheikhs/ahmed.jpg',
            rating: 4.9,
            recitation_style: 'Hafs',
            teaching_type: 'Male & Female',
            starting_price: '200 EGP / session',
            intro_video_url: 'https://shike.developteam.site/uploads/intro/ahmed.mp4',
          },
        ],
        pagination: {
          current_page: 1,
  ...
  if (m === 'get' && p.includes('/v1/student/search/sheikhs')) {
    return {
      sheikhs: [
        {
          id: 'teacher_quran_ahmed_hassan',
  ...
  if (m === 'get' && p.includes('/v1/student/search/courses')) {
    return {
      courses: [
        {
          id: 'clxcourse123',
          name: 'Tajweed Basics',
          description: 'Learn correct pronunciation and recitation rules.',
          image: null,
          price: '500 جنيه',
          duration: '8 أسابيع',
          lessonsCount: 24,
          studentsCount: 15,
          rating: 4.8,
          sheikhs: [{ id: 'teacher_1', name: 'Ahmed Hassan' }],
        },
      ],
      pagination: {
        page: 1,
  ...
  if (m === 'get' && p.includes('/v1/student/my-courses')) {
    return [
      {
        id: 'course-uuid',
        name: 'Tajweed Basics',
        name_ar: 'أساسيات التجويد',
        description: 'Learn correct pronunciation and recitation rules.',
        full_description: 'A comprehensive course covering all Tajweed rules...',
        image: 'https://example.com/course.jpg',
        intro_video_url: 'https://example.com/intro.mp4',
        intro_video_thumbnail: 'https://example.com/thumb.jpg',
        price: 500,
        duration: '3 months',
        category: 'Tajweed',
        level: 'BEGINNER',
        ...
        completed_at: null,
        sheikhs: [
          { id: 'teacher-uuid', name: 'Ahmed Hassan', image: null, bio: 'Certified Quran teacher', specialization: 'Tajweed', rating: 4.9, experience: 10 },
        ],
        lessons: [
          {
            id: 'lesson-uuid',
            ...
            videos_count: 2,
            videos: [
              { id: 'video-uuid', title: 'Part 1', video_url: 'https://example.com/v1.mp4', thumbnail_url: null, duration_seconds: 900, order: 1, is_completed: true, watch_progress: 100 },
            ],
          },
        ],
      },
  ...
  if (m === 'get' && p.includes('/v1/student/my-bookings')) {
    return {
      bookings: [
        {
          id: 'booking-uuid',
          ...
          sessions: [
            { id: 'session-uuid', scheduledDate: '2026-03-05', startTime: '10:00', endTime: '12:00', status: 'CONFIRMED', meetingLink: null },
          ],
        },
      ],
  ...
  if (m === 'get' && p.includes('/v1/student/notifications') && !p.includes('/read') && !p.includes('/read-all')) {
    return [
      {
        id: 'notif_001',
        userId: 'student_001',
        ...
        message: 'Your lesson with Sheikh Ahmed is confirmed for tomorrow at 10:00.',
        data: {},
...
function isMobileRelevantPath(routePath) {
  const p = String(routePath || '').toLowerCase();
  if (p === '/health' || p === '/mobile/health') return true;
  ...
  return (
    ...
    p.startsWith('/student-subscriptions') ||
...
function isMobileRelevantOperation(routePath, method) {
  const p = String(routePath || '').toLowerCase();
  const m = String(method || '').toLowerCase();
  if (!isMobileRelevantPath(routePath)) return false;

  // Internal callbacks / webhooks are not part of mobile client flow.
  if (p.includes('/webhook')) return false;

  // Public docs for mobile should avoid non-mobile payment internals.
  if (p.startsWith('/payments') && !p.includes('/fawry') && !p.includes('/bookings/')) return false;

  // Keep only methods used by client-facing flows.
  return ['get', 'post', 'put', 'patch', 'delete'].includes(m);
}

function inferMobileOperationDescription(routePath, method) {
  const p = String(routePath || '').toLowerCase();
  const m = String(method || '').toLowerCase();

  if (p === '/auth/login' && m === 'post') return 'Logs in the mobile user and returns access/refresh tokens used in all authenticated requests.';
  if (p === '/auth/register' && m === 'post') return 'Registers a new mobile user account.';
  if (p === '/auth/me' && m === 'get') return 'Returns the authenticated user profile for app bootstrap.';
  if (p.startsWith('/v1/quran-sheikhs') || p.startsWith('/mobile/quran-sheikhs')) {
    if (p.endsWith('/bookable')) return 'Lists bookable Quran sheikhs (FULL_TEACHER) for mobile discovery.';
    if (p.includes('/availability')) return 'Returns available booking windows for a sheikh after excluding occupied slots.';
    if (p.endsWith('/me/schedules') && m === 'get') return 'Returns the authenticated sheikh active schedules (mobile sheikh mode).';
    if (p.endsWith('/me/schedules') && m === 'post') return 'Creates one or many schedule slots for the authenticated sheikh.';
    return 'Quran sheikh discovery/details endpoint for mobile flow.';
  }
  if (p === '/bookings' && m === 'post') return 'Creates a single booking and returns booking data. If the student has an active package with this sheikh, returns package timeline too.';
  if (p === '/bookings/my-bookings' && m === 'get') return 'Returns the authenticated user bookings list (student/teacher depending on role).';
  if (p.includes('/bookings/teacher/') && p.endsWith('/subscription-packages')) return 'Returns available subscription packages for a specific sheikh and current student subscription (if any).';
  if (p.startsWith('/student-subscriptions/subscribe') && m === 'post') return 'Creates a student subscription, generates package bookings, and returns booked schedule/sessions timeline.';
  if (p.startsWith('/student-subscriptions/my-active') && m === 'get') return 'Returns currently active subscription for the authenticated student.';
  if (p.startsWith('/student-subscriptions/my-subscriptions') && m === 'get') return 'Returns all subscriptions for the authenticated student.';
  if (p.startsWith('/payments/fawry/checkout-link') && m === 'post') return 'Creates Fawry checkout link (CARD/MWALLET) for a booking.';
  if (p.startsWith('/payments/fawry/reference-number') && m === 'post') return 'Generates PayAtFawry reference number for over-the-counter payment.';
  if (p.startsWith('/payments/fawry/status/') && m === 'get') return 'Checks current Fawry payment status by merchant reference number.';
  if (p.startsWith('/video/session/create') && m === 'post') return 'Creates or reuses a video session room and returns RTC token.';
  if (p.startsWith('/video/session/token/') && m === 'get') return 'Returns RTC token for an existing booking session. Student is blocked before session start time.';
  if (p.startsWith('/video/session/end') && m === 'post') return 'Ends a video session and marks booking/session state accordingly.';
  if (p.startsWith('/video/session/history') && m === 'get') return 'Returns historical data for a specific booking session.';
  if (p === '/health' || p === '/mobile/health') return 'Health endpoint used by mobile app for connectivity checks.';
  if (p.includes('/v1/student/search/sheikhs')) return 'Returns a paginated list of bookable Quran sheikhs with optional search by name (student flow).';
  if (p.includes('/v1/student/search/courses')) return 'Returns a paginated list of published courses with optional search by title or description (student flow).';
  if (p.includes('/v1/student/notifications') && p.endsWith('/notifications') && m === 'get') return 'Returns the authenticated student\'s notifications with optional unread-only filter and pagination (page, limit).';
  if (p.includes('/v1/student/notifications/read-all') && m === 'patch') return 'Marks all of the authenticated student\'s notifications as read.';
  if (p.includes('/v1/student/notifications/') && p.includes('/read') && m === 'patch') return 'Marks a single notification as read.';
  if (p.includes('/v1/student/notifications/') && m === 'delete') return 'Deletes one of the authenticated student\'s notifications.';
  if (p.includes('/v1/student/my-courses') && m === 'get') return 'Returns FULL details of enrolled courses: course info (name, description, price, level, category, image), all lessons with videos, per-video watch progress, sheikhs (bio, specialization, experience, rating), and overall student progress.';
  if (p.includes('/v1/student/my-bookings') && m === 'get') return 'Returns all bookings for the student with sheikh details (name, image, rating, specialization) and whether the student is currently subscribed to each sheikh (is_subscribed). Supports pagination.';

  return null;
}

function inferMobileOperationSummary(routePath, method) {
  const p = String(routePath || '').toLowerCase();
  const m = String(method || '').toLowerCase();
  if (p === '/auth/login' && m === 'post') return 'Login (Mobile)';
  if (p === '/auth/register' && m === 'post') return 'Register User';
  if (p === '/auth/me' && m === 'get') return 'Get Current User';
  if (p.endsWith('/bookable') && p.includes('quran-sheikhs')) return 'List Bookable Quran Sheikhs';
  if (p.includes('/quran-sheikhs') && p.includes('/availability')) return 'Get Sheikh Availability Windows';
  if (p === '/bookings' && m === 'post') return 'Create Booking';
  if (p === '/bookings/my-bookings' && m === 'get') return 'List My Bookings';
  if (p.includes('/subscription-packages') && m === 'get') return 'Get Sheikh Subscription Packages';
  if (p.startsWith('/student-subscriptions/subscribe') && m === 'post') return 'Subscribe And Generate Schedule';
  if (p.startsWith('/student-subscriptions/my-active') && m === 'get') return 'Get Active Subscription';
  if (p.startsWith('/student-subscriptions/my-subscriptions') && m === 'get') return 'List My Subscriptions';
  if (p.startsWith('/payments/fawry/checkout-link') && m === 'post') return 'Create Fawry Checkout Link';
  if (p.startsWith('/payments/fawry/reference-number') && m === 'post') return 'Generate PayAtFawry Reference';
  if (p.startsWith('/payments/fawry/status/') && m === 'get') return 'Check Fawry Payment Status';
  if (p.startsWith('/video/session/create') && m === 'post') return 'Create/Re-use Video Session';
  if (p.startsWith('/video/session/token/') && m === 'get') return 'Get Session RTC Token';
  if (p.startsWith('/video/session/end') && m === 'post') return 'End Video Session';
  if (p.startsWith('/video/session/history') && m === 'get') return 'Get Session History';
  if (p === '/health' || p === '/mobile/health') return 'Health Check';
  if (p.includes('/v1/student/search/sheikhs')) return 'Search Sheikhs (Student)';
  if (p.includes('/v1/student/search/courses')) return 'Search Courses (Student)';
  if (p.includes('/v1/student/notifications') && !p.includes('/read') && !p.includes('/read-all') && m === 'get') return 'List My Notifications (Student)';
  if (p.includes('/v1/student/notifications/read-all') && m === 'patch') return 'Mark All Notifications Read (Student)';
  if (p.includes('/v1/student/notifications/') && p.includes('/read') && m === 'patch') return 'Mark Notification Read (Student)';
  if (p.includes('/v1/student/notifications/') && m === 'delete') return 'Delete Notification (Student)';
  if (p.includes('/v1/student/my-courses') && m === 'get') return 'My Enrolled Courses – Full Details (lessons, videos, sheikhs, progress)';
  if (p.includes('/v1/student/my-bookings') && m === 'get') return 'My Bookings (Student)';
  return null;
}

function pruneAndAnnotateForMobile(spec) {
  const filteredPaths = {};
  ...
  for (const [routePath, pathItem] of Object.entries(spec.paths || {})) {
    ...
    for (const method of httpMethods) {
      ...
      if (!isMobileRelevantOperation(routePath, method)) continue;

      const mobileSummary = inferMobileOperationSummary(routePath, method);
      if (mobileSummary) {
        operation.summary = mobileSummary;
      }

      const desc = inferMobileOperationDescription(routePath, method);
      if (desc) {
    ...
    if (Object.keys(nextPathItem).length > 0) {
      filteredPaths[routePath] = nextPathItem;
    }
  }

  // Remove duplicated quran-sheikhs aliases in mobile docs and keep /mobile/*
  for (const [routePath, pathItem] of Object.entries({ ...filteredPaths })) {
    ...
    if (filteredPaths[mobileAliasPath]) {
      delete filteredPaths[routePath];
    } else {
      filteredPaths[mobileAliasPath] = pathItem;
      delete filteredPaths[routePath];
    }
  }

  spec.paths = filteredPaths;
  spec.info.title = `${spec.info.title} (Mobile)`;
  spec.info.description =
    'Mobile-focused API documentation. Contains only client flow endpoints (auth, discovery, bookings, subscriptions, payments, video sessions), with duplicate/non-mobile endpoints removed.';
  spec.tags = (spec.tags || [])
    .filter((tag) => ['auth', 'users', 'bookings', 'teachers', 'reviews', 'payments', 'video', 'student-subscriptions', 'quran-sheikhs', 'courses', 'lessons', 'health', 'student'].includes(String(tag.name || '').toLowerCase()));
}

function buildSwaggerSpec({ port, profile = 'full' }) {
      ...
      if (requestBody && requestBody.content) {
        ...
        for (const [contentType, mediaType] of Object.entries(requestBody.content)) {
          ...
          if (contentType === 'application/json') {
            ...
            if (
              routePath.includes('/student-subscriptions/packages') &&
              (method === 'post' || method === 'put') &&
              mediaType.example &&
              typeof mediaType.example === 'object' &&
              !Array.isArray(mediaType.example)
            ) {
              if (mediaType.example.sessionsPerMonth === undefined) {
                mediaType.example.sessionsPerMonth =
                  mediaType.example.totalSessions ??
                mediaType.example.maxBookings ??
                8;
              }
            }
            if (
              routePath.includes('/student-subscriptions/subscribe') &&
              method === 'post' &&
              mediaType.example &&
              typeof mediaType.example === 'object' &&
              !Array.isArray(mediaType.example)
            ) {
              const selectedSlots = Array.isArray(mediaType.example.selectedSlots) ? mediaType.example.selectedSlots : [];
              const hasScheduleId = selectedSlots.some((slot) => slot && typeof slot === 'object' && slot.scheduleId);
              if (!hasScheduleId) {
                mediaType.example.selectedSlots = [
                  { scheduleId: 'schedule_sat_0330_0530' },
                  { scheduleId: 'schedule_tue_1900_2100' },
                ];
              }
  ...
  if (profile === 'mobile') {
    pruneAndAnnotateForMobile(spec);
  }

  return spec;
}

function buildMobileSwaggerSpec({ port }) {
  return buildSwaggerSpec({ port, profile: 'mobile' });
}

module.exports = { buildSwaggerSpec, buildMobileSwaggerSpec };
