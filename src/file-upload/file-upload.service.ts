...
export class FileUploadService {
  ...
  async uploadFile(file: Express.Multer.File, folder: string = 'uploads'): Promise<string> {
    ...
    const maxSize = parseInt(this.config.get<string>('MAX_FILE_SIZE') || '5242880');
    if (file.size > maxSize) {
      throw new BadRequestException('File size exceeds maximum allowed size');
    }

    const allowedTypes = (this.config.get<string>('ALLOWED_FILE_TYPES') || 'jpg,jpeg,png,pdf').split(',');
