...
export class ExamService {
  ...
  async addQuestion(examId: string, dto: AddQuestionDto, teacherId: string) {
  ...
  async publishExam(examId: string, teacherId: string) {
    ...
    if (exam.teacher.userId !== teacherId) {
      throw new ForbiddenException('You are not authorized to publish this exam');
    }
  ...
  async getExam(examId: string, userId: string) {
    ...
    if (user.role === 'STUDENT') {
      ...
      if (submission) {
        // Return exam with answers if already submitted
        return {
          ...exam,
          submission,
          canSubmit: false,
    ...
    // Hide correct answers if not teacher
  ...
  async submitExam(examId: string, dto: SubmitExamDto, studentId: string) {
    const exam = await this.prisma.exam.findUnique({
      where: { id: examId },
    ...
    // Create answers and auto-grade MCQ/TrueFalse
    let totalMarks = 0;
    const answers = [];
  ...
  async gradeExam(examId: string, submissionId: string, teacherId: string) {
    const exam = await this.prisma.exam.findUnique({
      where: { id: examId },
      include: { teacher: true },
    });
